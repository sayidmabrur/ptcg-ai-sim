...
#: The learner's spec: the same two temporal encoders, over 30 frames instead of 60.
#:
#: Kept separate from ``DEFAULT_SPEC`` on purpose. The three frozen BC agents are both the
#: curriculum and the yardstick every win rate in FINDINGS.md is measured against, and they
#: were fitted at 60 — feeding them a truncated chain is a distribution shift on the
#: benchmark itself, which would move the number being compared rather than the policy being
#: measured. So they keep 60 and only the learner sees 30.
#:
#: Nothing in the network changes shape: ``ReversePositionalEmbedding`` is indexed by
#: recency with ``max_len=64``, so a 30-long chain uses positions 0-29 of the same table and
#: every checkpoint still loads. What does change is cost — a decision re-featurises the
#: chain, so it is the quadratic term in the ~6.4 ms ``transform`` spends per decision, and
#: halving the window roughly halves it.
