...
def transform_pokemon(pokemon):
    ...
    #
    # The skill fields are requested too, and are not dead here: 12 special
    # Energy cards carry rules text (Legacy Energy's third prize card,
    # Enriching Energy's draw 4, Mist Energy's effect prevention), and an
    # attached special Energy changes what the Pokémon it is on can do. Leaving
    # them out would hide that from the board encoder specifically.
    fields.update(
...
#: in ``data/policy_decisions_lucario.parquet`` (noted per field) so a rare
...
_NORMALIZATION_CAPS = {
    ...
    # scan of data/policy_decisions_lucario.parquet), so headroom above that.
